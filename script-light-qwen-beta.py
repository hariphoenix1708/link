...
# ---- FIX: install uv WITHOUT using pip ----
subprocess.run([
    "bash", "-c",
    "command -v uv >/dev/null 2>&1 || curl -Ls https://astral.sh/uv/install.sh | sh"
])
...
# Install pyngrok inside venv
subprocess.run(["uv", "pip", "install", "--python", VENV_PY, "pyngrok"])
...
if INSTALL_DEPS:
    ...
    # Bind uv to venv python
    #subprocess.run(["uv", "pip", "install", "--python", VENV_PY, "setuptools==80", "pip==25"])
    #subprocess.run(["uv", "pip", "uninstall", "--python", VENV_PY, "numpy", "pandas", "scikit-learn"])
    subprocess.run(["uv", "cache", "clean"])
    #subprocess.run(["uv", "pip", "install", "--python", VENV_PY,
                    #"numpy==1.26.4", "pandas==2.2.2", "scikit-learn==1.5.1"])
    subprocess.run(["uv", "pip", "install", "--python", VENV_PY, "-r", "requirements.txt"])
    subprocess.run([
        "aria2c",
