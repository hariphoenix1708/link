...
if INSTALL_DEPS:
    ...
    subprocess.run(["apt-get", "update", "-y"]); subprocess.run(["apt-get", "install", "-y", "aria2", "uv", "git", "tqdm", "python3-pip"])
    subprocess.run(["apt", "--fix-broken", "install", "-y"])
    subprocess.run(["apt", "-y", "install", "-qq", "aria2"])
    
from tqdm import tqdm
def robust_urlretrieve(url, filename):
    print(f"Downloading: {url}")
    try:
        robust_urlretrieve(url, filename)
...
# Launch WebUI
LAUNCH_CMD = f"python launch.py {share} --gpu-device-id 0 --xformers --cuda-stream --always-gpu --pin-shared-memory --api --listen --enable-insecure-extension-access {auth} --disable-console-progressbars --no-hashing --precision autocast --upcast-sampling"
print(datetime.now(), end=" | "); print(f"Starting WebUI with command:\n{LAUNCH_CMD}")
