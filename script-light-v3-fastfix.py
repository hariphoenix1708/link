...
if INSTALL_DEPS:
    ...
    subprocess.run(["sudo", "apt-get", "--fix-broken", "install", "-y"], check=True)
    subprocess.run(["sudo", "apt-get", "-y", "install", "-qq", "aria2"], check=True)
...
def download_from_txt(txt_url, token=CIVITAI_TOKEN):
    ...
    # Parallel HuggingFace / direct downloads
    if huggingface_jobs:
        ...
        with ThreadPoolExecutor(max_workers=HF_WORKERS) as executor:
            futures = [
                executor.submit(
                    run_with_retry,
                    download_huggingface_fast,
                    url,
                    output_dir,
                    label=f"HF:{os.path.basename(urlparse(url).path) or 'download'}",
                )
                for url, output_dir in huggingface_jobs
            ]
...
try:
    ...
    if Ngrok_domain:
        ...
        tunnel = ngrok.connect(7860, bind_tls=True, pyngrok_config=ngrok_config)
...
# Launch WebUI
# Backend-only: accessed only through ngrok.
LAUNCH_CMD = (
