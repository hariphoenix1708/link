...
def civitai_download_to_path(download_url: str, output_dir: str, token: str):
    ...
    # Keep the full original Civitai query string from batch.txt.
    # This preserves type/format/size/fp parameters that select the exact file variant.
    ...
    # Use a session so cookies and redirect handling stay consistent.
    session = requests.Session()

    # First request: obtain the signed redirect URL.
    resp = session.get(download_url, headers=api_headers, allow_redirects=False, timeout=120)
    if resp.status_code not in (301, 302, 303, 307, 308):
        resp.raise_for_status()

    redirect_url = resp.headers.get("Location")
    if not redirect_url:
        raise RuntimeError(f"Civitai download did not return a redirect URL: {download_url}")

    redirect_url = urljoin(download_url, redirect_url)

    # The signed file URL usually works best without the Authorization header.
    # If the first attempt fails, retry once with the token header too.
    ...
    last_error = None
    for header_index, download_headers in enumerate(download_header_sets, start=1):
        try:
            with session.get(redirect_url, headers=download_headers, stream=True, timeout=300) as r:
                if r.status_code >= 400:
                    raise requests.HTTPError(f"{r.status_code} {r.reason} for url: {r.url}", response=r)
                ...
                filename = (
                    ...
                    or f"{extract_civitai_model_id(download_url) or 'downloaded_file'}.bin"
                )
            ...
            last_error = e
            if header_index < len(download_header_sets):
                time.sleep(2)
...
def download_from_txt(txt_url, token=CIVITAI_TOKEN):
    ...
    for raw_line in lines:
        ...
        if current_dir and line.startswith("$"):
            # Only $ lines are model downloads; everything else is ignored.
            link = line[1:].strip()
    ...
    if civitai_jobs:
        ...
        for download_url, output_dir in civitai_jobs:
            civitai_id = extract_civitai_model_id(download_url) or download_url
            run_with_retry(
                civitai_download_to_path,
                download_url,
                output_dir,
                token,
                label=f"Civitai:{civitai_id}",
            )
