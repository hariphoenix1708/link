...
def civitai_download_to_path(model_id: str, output_dir: str, token: str):
    if not token:
        raise RuntimeError(
    ...
    # First request: obtain redirect URL
    resp = requests.get(api_url, headers=headers, allow_redirects=False, timeout=120)
    if resp.status_code not in (301, 302, 303, 307, 308):
        resp.raise_for_status()

    redirect_url = resp.headers.get("Location")
    if not redirect_url:
        raise RuntimeError(f"Civitai:{model_id} missing redirect location")
...
def download_from_txt(txt_url, token=CIVITAI_TOKEN):
    ...
    if civitai_jobs:
        ...
        for model_id, output_dir in civitai_jobs:
            run_with_retry(
