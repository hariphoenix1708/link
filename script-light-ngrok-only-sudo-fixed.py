...
# Civitai token: Kaggle secret preferred when available, then environment fallback,
# then the working default from the Kaggle version.
DEFAULT_CIVITAI_TOKEN = "52c41327b3f7485898faf9e92a29e008"
...
try:
    ...
    CIVITAI_TOKEN = os.getenv("CIVITAI_TOKEN", DEFAULT_CIVITAI_TOKEN).strip() or DEFAULT_CIVITAI_TOKEN
...
LIGHTNING_ENV = any(key.startswith("LIGHTNING_") for key in os.environ)
...
if INSTALL_DEPS:
    ...
    # Lightning AI explicitly supports using apt-get and pip inside a Studio.
    subprocess.run(["apt-get", "update", "-y"], check=True)
    subprocess.run(["apt-get", "--fix-broken", "install", "-y"], check=False)
    subprocess.run(["apt-get", "-y", "install", "-qq", "aria2"], check=True)

    subprocess.run([sys.executable, "-m", "pip", "install", "--upgrade", "setuptools==80", "pip==25"], check=True)
...
def civitai_download_to_path(download_url: str, output_dir: str, token: str):
    if not token:
        raise RuntimeError(
            "CIVITAI_TOKEN is missing. Add it as an environment variable or notebook secret."
        )
    ...
    # First request: Civitai API endpoint. This may return a redirect to a signed file URL.
    resp = requests.get(clean_url, headers=api_headers, allow_redirects=False, timeout=120)
    ...
    # Do NOT send the Civitai bearer token here. The signature is already embedded in the URL.
    download_headers = {
        "User-Agent": USER_AGENT,
        "Referer": "https://civitai.com/",
        "Accept": "*/*",
    ...
    try:
        r = requests.get(redirect_url, headers=download_headers, stream=True, timeout=300)
        r.raise_for_status()
    except requests.HTTPError as first_error:
        # Some signed URLs are sensitive to extra headers; retry once with the bare minimum.
        try:
            r = requests.get(redirect_url, stream=True, timeout=300)
            r.raise_for_status()
        except Exception:
            raise first_error
...
# NGROK / Lightning CONFIG
COMIC = True  # hk1708
MIDGE = False  # vk872

if COMIC:
    NGROK_AUTHTOKEN = os.getenv("NGROK_AUTHTOKEN", "1ec4Az5OT9kYyABKxKnhLUl5jpH_7h6HjLDYkybGRS93M5aqG")
    Ngrok_domain = os.getenv("NGROK_DOMAIN", "comic-caribou-frankly.ngrok-free.app")
elif MIDGE:
    NGROK_AUTHTOKEN = os.getenv("NGROK_AUTHTOKEN", "2SVfPekz6a7YTrM0V6FoZBeQblN_WV4nWZsvjRRAjSMubeS")
    Ngrok_domain = os.getenv("NGROK_DOMAIN", "midge-major-falcon.ngrok-free.app")
else:
    NGROK_AUTHTOKEN = os.getenv("NGROK_AUTHTOKEN")
    Ngrok_domain = os.getenv("NGROK_DOMAIN")

auth = f"--gradio-auth {USERNAME}:{PASSWORD}" if USE_USERNAME_AND_PASSWORD and USERNAME and PASSWORD else ""

if LIGHTNING_ENV:
    print("Lightning AI Studio detected: the app listens on port 7860; use the Studio public link / ports plugin if you want to expose it.")
else:
    print("Non-Lightning environment detected: ngrok/share fallback remains available.")

# NGROK Tunnel (ngrok-only backend)
if not NGROK_AUTHTOKEN:
    raise RuntimeError("NGROK_AUTHTOKEN is missing. Set it in the notebook or as an environment variable.")

try:
    ngrok.kill()
    ngrok_config = conf.PyngrokConfig(auth_token=NGROK_AUTHTOKEN)
    if Ngrok_domain:
        ...
        tunnel = ngrok.connect(7860, pyngrok_config=ngrok_config)
        public_url = tunnel.public_url

    print(f"NGROK Link ✅: {public_url}")
except Exception as e:
    raise RuntimeError(f"NGROK Error: {e}") from e

# Launch WebUI
# Backend-only: no share fallback; ngrok exposes the local 7860 port.
LAUNCH_CMD = (
    f"python launch.py --api --listen --cuda-stream --cuda-malloc "
    f"--pin-shared-memory --always-gpu --force-channels-last "
    f"--attention-pytorch --enable-insecure-extension-access {auth} "
    f"--disable-console-progressbars"
)
